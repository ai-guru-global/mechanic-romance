...
- ✅ **一个场景 = 一份任务 / 观测 / 评估齐备、可照着复现的设计文档**
- ✅ **一句话原则 = 看完即懂，讲完即用**

**它是什么**：
- 🔍 **可检索的语料库**——论文方法不再散落各处
- 🎬 **可讲解的场景工作台**——把抽象的 SOTA 数字拆成说得清的任务与评估（可跑实现在 M3 逐步落地）
- 📓 **可回放的研究日志**——从选题到跑通的全过程
...
> 每个场景用统一模板定义任务、观测、动作、评估、硬件——**可复现、可比较、可讲解**。当前 8 个场景均为**设计文档**（状态见各场景 README），可跑的训练 / 评估脚本随 M3「实验复现」落地。
...
**从场景定义到可跑实验**（以 demo 01 为例）：

当前 8 个场景均为**设计文档**，仓库内暂无可直接运行的训练 / 评估脚本（落地计划见 [roadmap](docs/roadmap.md) M3）。在此之前，请按场景「方法」一节指向的原始方法官方仓库复现，例如 demo 01 对应 [Diffusion Policy 官方实现](https://github.com/real-stanford/diffusion_policy)：

```bash
# 1. 阅读场景定义，明确任务 / 观测 / 评估协议
cat demo/scenarios/01-扩散策略-桌面堆叠/README.md

# 2. 用官方实现跑通原始环境，再对照场景文档替换任务设置
...
| **M2 · 场景定义** | 落地 8 大类 8 个场景定义；首个场景推进到最小可跑复现 | 🟡 进行中（8 份定义已起草） | — |
